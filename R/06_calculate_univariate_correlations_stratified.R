#' Run Regressions with Clocks and Cells Stratified by Categorical Variables and Save Results
#'
#' This function performs regressions with each clock variable against each cell variable,
#' including specified covariates, stratified by levels of a categorical variable. It calculates
#' the confidence intervals for the cell estimates and saves the results to a CSV file.
#'
...
#' @param categorical_variables A character vector of categorical variables to stratify the analysis.
...
calculate_univariate_correlations_stratified <- function(cell_clock_df, control_covariates = 1, categorical_variables, study) {
  ...
  univariate_correlations_filename_stratified <- paste0("cellclockR_output/Tables/", study, "_clock_cell_univariate_correlations_stratified.csv")

  # Save the summary data to CSV
  write_csv(x = results_df, file = univariate_correlations_filename_stratified)
